...
seq_length = 3
cn_seq = len(c_train_data)
cX = numpy.reshape(numpy.array(c_train_data), (cn_seq, seq_length, 300))
cy = numpy.array(c_train_labels)

# define the LSTM model
model = Sequential()
model.add(LSTM(1200, input_shape=(cX.shape[1:])))
model.add(Dropout(0.3))
model.add(Dense(cy.shape[1], activation='softmax'))
...
# model.fit(cX, cy, nb_epoch=50, batch_size=32, callbacks=callbacks_list)
...
filename = "weights-improvement-49--417.3199-bigger.hdf5"
...
while True:
    ...
    def lstm(pattern):
        for i in range(3):
            ...
            result = c_model.most_similar(positive=[prediction], topn=1)
            word, prob = result[0]
            # convert returned word back to a vector
            # set this vector as the next vector in pattern
            wordVect = w2v_dict_clinton[word]
